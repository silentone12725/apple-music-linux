...
func StartWrapper() (*Wrapper, error) {
	binaryPath, err := resolveBinaryPath()
	if err != nil {
		return nil, err
	}

	log.Printf("[WrapperProc] Starting wrapper at: %s", binaryPath)
...
// resolveBinaryPath determines the correct path to the wrapper binary.
// In dev/production, it looks for bin/wrapper relative to the executable.
func resolveBinaryPath() (string, error) {
	// Prefer an explicit env override (useful for CI or custom installs)
	if envPath := os.Getenv("WRAPPER_BIN"); envPath != "" {
		return envPath, nil
	}
	...
	// Resolve symlinks (e.g. go run temp binary in /tmp)
	...
	if runtime.GOOS == "windows" {
		binaryName = "wrapper.exe"
	}

	// <project-root>/bin/wrapper
	return filepath.Join(filepath.Dir(exe), "..", "bin", binaryName), nil
}
