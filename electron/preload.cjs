...
contextBridge.exposeInMainWorld('amlBridge', {
    ...
    setBgBlur:      (b)         => ipcRenderer.send('view:bg-blur', b),
    setTweak:       (k, v)      => ipcRenderer.send('view:tweak', k, v),
    getThemeInfo:      ()         => ipcRenderer.invoke('theme:get-info'),
    setThemeMode:      (m)        => ipcRenderer.send('theme:set-mode', m),
    setThemePalette:   (k, v)     => ipcRenderer.send('theme:set-palette', k, v),
    resetThemePalette: ()         => ipcRenderer.invoke('theme:reset-palette'),
    importThemeCss:    ()         => ipcRenderer.invoke('theme:import-css'),
    saveThemePreset:   (name)     => ipcRenderer.invoke('theme:save-preset', name),
    deleteThemePreset: (name)     => ipcRenderer.send('theme:delete-preset', name),
    applyThemePreset:  (name)     => ipcRenderer.send('theme:apply-preset', name),
    exportThemePreset: (name)     => ipcRenderer.invoke('theme:export-preset', name),
