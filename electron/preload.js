...
// ── Expose wrapper IPC to the renderer ───────────────────────────────────────
// contextBridge is Electron's secure channel between the isolated preload
// world and the page's JS. Replaces Wails' window.go bindings entirely.
contextBridge.exposeInMainWorld('aml', {
    wrapperLogs: () => ipcRenderer.invoke('wrapper:getLogs'),
    wrapperLogin: (loginStr) => ipcRenderer.invoke('wrapper:login', loginStr),
    onWrapperLog: (cb) => {
        ipcRenderer.on('wrapper:log', (event, line) => cb(line));
    },
});

// ── All DOM work runs once the page is ready ─────────────────────────────────
// This runs exactly once per navigation — no 500ms re-injection loop needed.
...
function setupAppleMusicPage() {
    ...
    // ── Inject styles ──────────────────────────────────────────────────────
    if (!document.getElementById('aml-style')) {
        const style = document.createElement('style');
        style.id = 'aml-style';
        style.textContent = `
            /* KWin blur-behind: make page root transparent so compositor blur shows through */
            html, body {
                background: transparent !important;
            }
            /* The main app shell background — make it translucent so blur is visible */
            .web-app, [class*="app-container"], [class*="music-app"],
            .dt-root, [data-app-root], #root {
                background: transparent !important;
            }
            /* Reinstate the sidebar/player glass with slight tint so UI stays readable */
            .web-navigation, [class*="nav-container"], cwc-web-navigation {
    ...
    // ── Lossless icon (debounced MutationObserver) ─────────────────────────
    ...
    // ── Collapsible wrapper terminal ───────────────────────────────────────
    if (document.getElementById('aml-settings-bar')) return;

    const bar = document.createElement('button');
    bar.id = 'aml-settings-bar';
    bar.textContent = '⚙ Wrapper';
    bar.style.cssText = 'position:fixed;top:0;right:24px;z-index:2147483647;padding:6px 14px;' +
        'border:1px solid rgba(255,255,255,0.1);border-top:none;border-radius:0 0 10px 10px;' +
        'background:rgba(20,20,22,0.92);color:rgba(255,255,255,0.65);' +
        'font:600 11px -apple-system,sans-serif;letter-spacing:.02em;cursor:pointer;' +
        'backdrop-filter:blur(8px);transition:color .15s,background .15s';

    const panel = document.createElement('div');
    panel.id = 'aml-settings-panel';
    panel.style.cssText = 'position:fixed;top:28px;right:24px;z-index:2147483646;width:440px;' +
        'max-height:0;opacity:0;pointer-events:none;display:flex;flex-direction:column;overflow:hidden;' +
        'background:rgba(18,18,20,0.97);border:1px solid rgba(255,255,255,0.1);' +
        'border-radius:0 0 12px 12px;box-shadow:0 16px 48px rgba(0,0,0,.55);' +
        'font:12px/1.4 ui-monospace,SFMono-Regular,Menlo,monospace;' +
        'transition:max-height .22s ease,opacity .18s ease';

    const logEl = document.createElement('div');
    logEl.style.cssText = 'flex:1;min-height:220px;max-height:280px;overflow-y:auto;' +
        'padding:10px 12px;color:rgba(255,255,255,.8);white-space:pre-wrap;word-break:break-all';

    const inputRow = document.createElement('div');
    inputRow.style.cssText = 'display:flex;border-top:1px solid rgba(255,255,255,.08)';

    const input = document.createElement('input');
    input.placeholder = 'Apple ID:Password — press Enter to log in';
    input.style.cssText = 'flex:1;padding:9px 12px;background:transparent;border:none;' +
        'outline:none;color:#fff;font:inherit';

    const sendBtn = document.createElement('button');
    sendBtn.textContent = 'Send';
    sendBtn.style.cssText = 'padding:9px 14px;background:transparent;border:none;' +
    ...
    const renderLines = (lines) => {
        ...
        logEl.textContent += (logEl.textContent ? '\n' : '') + lines.slice(renderedCount).join('\n');
    ...
    const sendLogin = async () => {
        ...
        if (!text) return;
        input.value = '';
        try { await window.aml.wrapperLogin(text); }
        catch (e) { logEl.textContent += '\n[settings] ' + e; }
    };

    sendBtn.addEventListener('click', sendLogin);
    input.addEventListener('keydown', e => { if (e.key === 'Enter') sendLogin(); });

    let open = false;
    let pollHandle = null;
    const poll = async () => {
        try { renderLines(await window.aml.wrapperLogs()); } catch (_) {}
    };
    const setOpen = (next) => {
        open = next;
        ...
        if (open) { poll(); if (!pollHandle) pollHandle = setInterval(poll, 1000); }
        else if (pollHandle) { clearInterval(pollHandle); pollHandle = null; }
    };
    bar.addEventListener('click', () => setOpen(!open));

    // Live push from main process.
    window.aml.onWrapperLog((line) => {
        renderedCount++;
        logEl.textContent += (logEl.textContent ? '\n' : '') + line;
...
// Run on every page navigation (not just initial load).
