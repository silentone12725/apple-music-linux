...
func promptForQuality(item SearchResultItem, token string) (string, error) {
	...
	// Check if Atmos is available for this item
	atmosAvailable := false
	if item.ID != "" {
		songResp, err := ampapi.GetSongResp(Config.Storefront, item.ID, Config.Language, token)
		if err == nil && songResp != nil && len(songResp.Data) > 0 {
			for _, trait := range songResp.Data[0].Attributes.AudioTraits {
				if trait == "atmos" {
					atmosAvailable = true
					break
				}
	...
	actionOptions := []string{"Download", "Stream"}
	...
	actionIndex := 0
	err = survey.AskOne(actionPrompt, &actionIndex)
	if err != nil {
		return "", nil
	}
	if actionIndex == 1 {
...
func handleSearch(searchType string, queryParts []string, token string) (string, error) {
	...
	for {
		...
		case "music-video":
			if searchResp.Results.MusicVideos != nil {
				for _, item := range searchResp.Results.MusicVideos.Data {
					...
					displayOptions = append(displayOptions, fmt.Sprintf("%s - %s", item.Attributes.Name, detail))
					items = append(items, SearchResultItem{Type: "MusicVideo", URL: item.Attributes.URL, ID: item.ID, Name: item.Attributes.Name})
				}
		...
		// For Music Videos, skip quality selection — caller handles action.
		if selectedItem.Type == "MusicVideo" {
...
func main() {
	...
	if err != nil {
		...
		fmt.Println("Using default config. Run with TUI settings to configure.")
		// Set safe defaults
		Config.Storefront = "us"
	...
	// Pre-scan os.Args for --api before pflag.Parse().  In API mode the bearer
	// token is provided per-request by the renderer (engine-playback.js sends
	// mk.developerToken in the POST body), so we can start without a global
	// token.  Skipping GetToken() also removes a blocking network round-trip that
	// would otherwise delay startup by 2-5 s on every engine launch.
	isAPIMode := func() bool {
		for _, a := range os.Args[1:] {
			if a == "--api" || strings.HasPrefix(a, "--api=") {
				return true
			}
		}
		return false
	...
	if !isAPIMode {
		...
		// Store the resolved token back into Config so the API server's bearerToken()
		// helper can read it.  Without this, the server would use the placeholder from
		// config.yaml and every catalog lookup would fail with an invalid auth header.
		Config.AuthorizationToken = "Bearer " + token
	}
	...
	pflag.StringVar(&stream_player, "player", "", "Force media player for streaming: mpv, vlc, ffplay")
	...
	// Warm the segment cache LRU index from existing on-disk files.
	runv3.WarmCache()

	// --api <port>: start the HTTP API server and block until interrupted.
	// The server exposes /api/v1/stream/* and /api/v1/metadata for Electron
	// and native players to consume.
	if api_port > 0 {
	...
	// Launch TUI if no args and no flags given
	if len(os.Args) == 1 {
		RunTUI(token)
		return
	}
	...
	for {
		for albumNum, urlRaw := range os.Args {
			...
			if strings.Contains(urlRaw, "/music-video/") {
				...
				if play_stream {
					// Streaming pipeline: player starts before download completes.
					if err := mvStreamToPlayer(albumId, token, storefront, Config.MediaUserToken); err != nil {
						fmt.Println("\u26A0 MV stream failed:", err)
						counter.Error++
					} else {
						counter.Success++
					}
				...
				_, err := mvDownloader(albumId, mvSaveDir, token, storefront, Config.MediaUserToken, nil)
				if err != nil {
					fmt.Println("\u26A0 Failed to dl MV:", err)
					counter.Error++
					continue
...
func extractMedia(b string, more_mode bool) (string, string, error) {
	...
	// Variant selection is shared with cmd/parity via utils/manifest so both
	// the legacy CLI and the parity tool resolve identically.
	...
	if !debug_mode && !more_mode && sel.Display != "" {
		fmt.Printf("%s\n", sel.Display)
	}
